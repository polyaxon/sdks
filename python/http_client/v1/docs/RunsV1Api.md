...
# **get_run_stats**
> object get_run_stats(owner, entity, uuid, offset=offset, limit=limit, sort=sort, query=query, bookmarks=bookmarks, mode=mode, kind=kind, aggregate=aggregate, groupby=groupby, trunc=trunc)
...
with polyaxon_sdk.ApiClient(configuration) as api_client:
    ...
    try:
        # Get run stats
        api_response = api_instance.get_run_stats(owner, entity, uuid, offset=offset, limit=limit, sort=sort, query=query, bookmarks=bookmarks, mode=mode, kind=kind, aggregate=aggregate, groupby=groupby, trunc=trunc)

...
generate-go-swagger:
	...
	$(GO) generate ./...
	$(DOCKER_RUN) find ./ -name "*.go" -exec $(DOCKER_PATH_AUTOGEN) -i --no-tlc --no-code -y 2018-2022 -c $(LICENSE_OWNER) -l apache {} \;
...
generate-js-swagger:
	...
	$(DOCKER_RUN) find ./ -name "*.js" -exec $(DOCKER_PATH_AUTOGEN) -i --no-tlc --no-code -y 2018-2022 -c $(LICENSE_OWNER) -l apache {} \;
...
generate-ts-swagger:
	...
	$(DOCKER_RUN) find ./ -name "*.ts" -exec $(DOCKER_PATH_AUTOGEN) -i --no-tlc --no-code -y 2018-2022 -c $(LICENSE_OWNER) -l apache {} \;

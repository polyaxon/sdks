...
export class AgentsV1Api extends runtime.BaseAPI {
    ...
    async collectAgentDataRaw(requestParameters: CollectAgentDataRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<object>> {
        ...
        const response = await this.request({
            path: `/streams/v1/{namespace}/{owner}/agents/{uuid}/collect`.replace(`{${"namespace"}}`, encodeURIComponent(String(requestParameters.namespace))).replace(`{${"owner"}}`, encodeURIComponent(String(requestParameters.owner))).replace(`{${"uuid"}}`, encodeURIComponent(String(requestParameters.uuid))),
            method: 'POST',
